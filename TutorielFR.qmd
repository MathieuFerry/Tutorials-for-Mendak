...
Mendak[^1] est une application R Shiny développée à but pédagogique pour introduire l’analyse statistique textuelle auprès d’étudiant·es qui ne manipulent de langage de programmation informatique (en particulier, R).
...
2.  Ces “axes” ou “dimensions” représentent les dimensions de variabilité du corpus : deux mots proches dans la représentation graphique sont plus souvent qu’en moyenne employés dans les mêmes textes, et deux mots éloignés dans la représentation sont moins souvent qu’en moyenne employés dans les mêmes textes. Par ailleurs, deux textes proches dans l’espace factoriel ont une proximité sémantique : ils se caractérisent par des mots communs qui leur sont typiques et sont différents des autres.
...
-   un Analyse Factorielle des Correspondances (AFC) où le tableau analysé est de la forme lignes (*jugements)* x colonnes (mots du corpus), donc un tableau de contingence. Cette méthode est **la plus classiquement utilisée pour en AGD lorsqu’on a affaire à des textes**. Cette petite [vidéo](https://www.youtube.com/watch?v=tEc5cmlQVdI&ab_channel=Fran%C3%A7oisHusson) est une introduction très rapide à l’AFC.
...
Comme à chaque fois, il y a au moins une classe qui rassemble beaucoup de segments de textes, dans l'exemple on a été assez maximaliste et on a gardé 9 classes.
...
Les clusters 4 et 5, assez proches les uns des autres, sont aussi les plus conséquents en nombres de segments de textes. Ces clusters ont davantage trait aux arguments que "fait valoir" le ou la requérant·e dans sa requête. Ces paragraphes relatent les conditions de travail auxquels ont été soumis les requérants et la manière dont ces conditions sont relatées puis médiées au cours du jugement pour être constitutive (ou non) de harcèlement moral.
